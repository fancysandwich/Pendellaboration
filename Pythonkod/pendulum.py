...
#import studentSol #This imports the file with entered solution.
import studentSol_withSolution #This imports another file with solutions implemented.
#OBS Make sure to also change line 97-106 according to studentsol_withSolution or studentSol
...
t = 0  # initial time
tmax = 30  # final time
#If no second argument, choose a default
try:
...
def handle_close(evt):
    # Handles close event. Plots position, velocity and energy (vs. time)
    fig2 = plt.figure()
...
# Creates a 1.2 * 1.2 window, for the animation of the pendulum motion
...
plt.axhline(y=0)  # draws a default horizontal line at y=0 that spans the x-range
plt.axvline(x=0)  # draws a default horizontal line at x=0 that spans the y-range
pendulum_line, = ax.plot([], [], lw=5) # preparing to plot the pendulum after creation of it.

def dtInterval(arg):
	#Input values are only allowed in a specific interval. Function checks whether the input is in [0.001, 0.05].
    try:
        dt = float(arg[2])
...
def init():
	#Sets the initial position of the pendulum.
